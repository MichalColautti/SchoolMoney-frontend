...
const MyClassItem = ({classInfo}) => {
    ...
    const {token} = useAuth();
    const {onDeleteFromClass} = useUserData();

    const {name, year, accessCode, fundraisers, children, imageId, id} = classInfo;

    const handleDeleteChildFromClass = async (childId) => {
        try {
            await deleteChildFromClass(id, childId, token);

            onDeleteFromClass(id, childId);
        } catch (error) {
            console.log(error);
        }
    }
                        ...
                        {children && children.map((student) => (
                            <div key={student.id} style={styles.studentItem}>
                                ...
                                <img
                                    ...
                                    alt={"delete"}
                                    onClick={() => handleDeleteChildFromClass(student.id)}
                                />
                            </div>
                        ))}
                    </div>
